...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('projects', function (Blueprint $table) {
            // Añadir solo las columnas que faltan, basándonos en la imagen y la información previa.
            // Ya existen: id, name, user_id, status, created_at, updated_at

            if (!Schema::hasColumn('projects', 'client_name')) {
                $table->string('client_name')->after('name');
            }
            if (!Schema::hasColumn('projects', 'nit')) {
               $table->string('nit')->after('client_name');
            }
